...
- Env: `OPENROUTER_API_KEY`; optional `AETHER_HOSTED_CLAUDE_BASE_URL` / `AETHER_HOSTED_CLAUDE_API_KEY` (default base `https://buzzai.cc/v1`); optional `AETHER_HOSTED_CHATGPT_BASE_URL` / `AETHER_HOSTED_CHATGPT_API_KEY` (default base `https://api.icodeeasy.cc/v1`; `AETHER_HOSTED_GPT_*` aliases accepted).
- Family routing: `claude-*` → Claude gateway then OpenRouter; ChatGPT (`gpt-*` / `o*`) → ChatGPT gateway then OpenRouter; other catalog ids → OpenRouter only.
